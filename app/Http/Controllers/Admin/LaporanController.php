...
class LaporanController extends Controller
{
    ...
    /**
     * Mendapatkan detail jawaban responden (untuk AJAX)
     */
    public function getResponseDetail($id)
{
    $answers = \App\Models\ResponseAnswer::with(['question', 'option'])
    ...
    foreach ($answers as $index => $answer) {
        $questionText = $answer->question->pertanyaan ?? '-';
        $optionText = $answer->option->teks_pilihan ?? null;
        $textAnswer = $answer->jawaban_teks ?? null;

        // Gunakan teks_pilihan jika multiple choice, atau jawaban_teks jika isian bebas
        $finalAnswer = $optionText ?? $textAnswer ?? '-';

        $detailHtml .= "
            <div class='answer-detail-item' style='margin-bottom: 12px;'>
                <div class='answer-detail-question' style='font-weight: 600; color: #374151;'>
                    <i class='fas fa-question-circle' style='color: #6366f1; margin-right: 8px;'></i>
                    Pertanyaan " . ($index + 1) . ": {$questionText}
                </div>
                <div class='answer-detail-response' style='margin-left: 26px; margin-top: 4px; color: #1f2937;'>
                    <i class='fas fa-comment-dots' style='color: #10b981; margin-right: 6px;'></i>
                    <strong>Jawaban:</strong> {$finalAnswer}
                </div>
            </div>
        ";
    }

    return response()->json(['html' => $detailHtml]);
}
