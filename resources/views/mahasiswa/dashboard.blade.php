...
@section('content')
    <div class="dashboard-container">
        ...
        <div class="welcome-section">
            <div class="welcome-content">
                <div>
                    {{-- Menggunakan data nama dari user yang login --}}
        ...
        <div class="stats-grid">
            ...
            {{-- Survey Selesai Diisi (Dinamis) --}}
            <div class="stat-card stat-success">
                <div class="stat-icon">
                    <i class="fas fa-check-circle"></i>
                </div>
                <div class="stat-content">
                    <h3 class="stat-number">{{ $totalCompleted }}</h3> 
                    <p class="stat-label">Survey Sudah Diisi</p>
                    <span class="stat-change positive">
                        <i class="fas fa-trophy"></i> Pertahankan Partisipasi!
                    </span>
                </div>
            </div>

            {{-- Survey Belum Diisi / Wajib (Dinamis) --}}
            <div class="stat-card stat-warning">
                ...
                <div class="stat-content">
                    ...
                    <span class="stat-change negative">
                        <i class="fas fa-exclamation-triangle"></i> Segera diselesaikan
                    </span>
                </div>
            </div>

            {{-- Poin Kontribusi (Dinamis) --}}
            <div class="stat-card stat-info">
                ...
                <div class="stat-content">
                    ...
                    <span class="stat-change positive">
                        <i class="fas fa-arrow-up"></i> Total nilai dari semua rating
                    </span>
                </div>
            </div>
        </div>

        {{-- Main Content: Daftar Survey yang Harus Diisi --}}
        <div class="content-grid-full">
            <div class="card card-table">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-clipboard-check"></i>
                        Survey yang Perlu Anda Isi ({{ $pendingSurveys->count() }} Survei)
                    </h3>
                    {{-- Mengarahkan ke halaman daftar survei penuh --}}
                    <a href="{{ route('mahasiswa.riwayat') }}" class="btn-link">Lihat Riwayat <i class="fas fa-arrow-right"></i></a>
                </div>
                            ...
                            <tbody>
                                @forelse ($pendingSurveys as $survey)
                                    <tr>
                                        ...
                                        {{-- Asumsi target default Mahasiswa TI karena tidak ada kolom target di surveys --}}
                                        <td>Teknik Informatika</td> 
                                        <td class="{{ $survey->tanggal_selesai->diffInDays(now()) <= 7 ? 'text-danger fw-bold' : '' }}">
                                            {{ $survey->tanggal_selesai->translatedFormat('d M Y') }}
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="action-buttons">
                                                {{-- Tombol menuju form pengisian --}}
                                                <a href="{{ route('mahasiswa.survey.show', $survey->uuid) }}" class="btn-action btn-fill" title="Isi Survey">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-4 text-success fw-bold">
                                            <i class="fas fa-check-circle me-2"></i> Selamat! Tidak ada survey wajib yang menunggu saat ini.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    <style>
        ...
        .btn-action { width: auto; height: auto; border: none; border-radius: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; font-size: 0.85rem; padding: 8px 16px; font-weight: 600; }
        .btn-fill { background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; }
        .btn-fill:hover { background: linear-gradient(135deg, #059669 0%, #047857 100%); }
        .btn-filled { background: #e5e7eb; color: #9ca3af; cursor: not-allowed; }
        @media (max-width: 1024px) { .content-grid { grid-template-columns: 1fr; } }
        @media (max-width: 768px) { .welcome-content { flex-direction: column; gap: 20px; text-align: center; } .stats-grid { grid-template-columns: 1fr; } .content-grid-full { grid-template-columns: 1fr; } .data-table th, .data-table td { min-width: 100px; } }
    </style>
@endsection
