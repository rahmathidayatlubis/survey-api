        ...
        <div class="modern-header" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); box-shadow: 0 12px 24px rgba(102, 126, 234, 0.4);">
            <div class="header-content">
                ...
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    ...
                    <button type="button" class="modern-action-btn" style="background-color: #10b981;" onclick="window.location.href='{{ route('admin.laporan.excel', $survey->uuid) }}'">
        ...
        <div class="content-grid-full">
            <div class="modern-card">
                                ...
                                <tr>
                                    ...
                                    <td class="nama-mahasiswa">
                                        <div class="profile-meta">
                                            ...
                                            <strong>{{ $response->user->nama ?? 'Anonymous' }}</strong>
                                        </div>
                                    </td>
                                    ...
                                    <td class="action-cell">
                                        <div class="action-buttons">
                                            <button type="button" class="action-icon-btn tooltip-btn" data-tooltip="Lihat Detail" onclick="showDetailModal({{ $response->id }}, '{{ $response->user->nama ?? 'Anonymous' }}')">
                ...
                <div class="modern-card-footer">
                    <span class="pagination-info">Menampilkan {{ $responses->firstItem() ?? 0 }} sampai {{ $responses->lastItem() ?? 0 }} dari {{ $responses->total() ?? 0 }} entri</span>
    ...
    function showDetailModal(responseId, respondentName) {
        ...
        contentEl.innerHTML = `
            <div style="text-align: center; padding: 40px; color: #6b7280;">
                ...
                <p>Sedang memuat detail jawaban...</p>
            </div>
        `;

        // Fetch detail via AJAX dari route controller
        fetch(`/admin/laporan/detail/${responseId}`)
            .then(response => response.json())
            .then(data => {
                // Pastikan response berisi HTML dari controller
                if (data.html) {
